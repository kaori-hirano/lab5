...
class(mls_cv) # okay now we redo the rest lol
...
Based on this plot, the optimum number of variables seems to be 7 because this is where the mse is the lowestand seems to taper off from there. These are being from the us, being defender, age, expected goals, being goalkeeper, games played, and minutes played. 

```{r mse-q1}
val_mat <- model.matrix(logs ~ usa + gk + df+ mf+fw+Age+MP+Starts+Min+Gls+Ast+PK+PKatt+CrdY+CrdR+xG+npxG+xAG+PrgC+PrgP+PrgR, data = d[val, ])
...
coef(regfit_best, 7) # here it says the best is 7...?
...
train <- sample(c(TRUE, FALSE), nrow(d),
    # replace = TRUE)
val <- (!train)
# don't want to use these because we set them up above? I think that's what the instructions are saying as opposed to the example code
#train <- mls_cv
#val <- mls_test
mls_fit_all <- lm(log(base_salary) ~ usa + gk + df+ mf+fw+Age+MP+Starts+Min+Gls+
                  Ast+PK+PKatt+CrdY+CrdR+xG+npxG+xAG+PrgC+PrgP+PrgR, d)

# sets up model
regfit_best <- regsubsets(logs ~ usa + gk + df+ mf+fw+Age+MP+Starts+Min+Gls+
                  Ast+PK+PKatt+CrdY+CrdR+xG+npxG+xAG+PrgC+PrgP+PrgR,
    data = d[train, ], nvmax = 21)
...
mean(val_errors_purrr)
```
The mean mse of the test set is .6363. 
...
```
The best lam is .0453. The test set MSE is .4669.
...
```
The optimal l is .0120. The test set MSE is .4986. The approach with the lowest test MSE is from ridge regression. 
...
defender, age, games played, minutes played, and expected goals. 
...
They are mostly the same, with lasso having a few more. I think the best subsets
was mostly what I would think of in general, like age and expected goals, 
while the lasso got into more details that might be something that 
someone who knows more about soccer may expect to influence pay. 
